...
@Component({
  ...
  styles: [`
    ...
    .gst-cell { display:inline-flex; flex-direction:column; gap:6px; align-items:stretch; min-width:120px; }
    /* Matches the standard app form-select (styles.scss) so it reads as an
       obvious dropdown, with the native caret kept via appearance:auto. */
    .gst-select {
      appearance:auto; -webkit-appearance:auto; width:120px; padding:8px 10px;
      border:1px solid var(--baseline); border-radius:9px;
      background:var(--surface); color:var(--ink); font:inherit; cursor:pointer;
    }
    .gst-manual {
      width:120px; padding:8px 10px; text-align:right; font:inherit;
