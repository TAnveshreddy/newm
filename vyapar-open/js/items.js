...
function openItemForm(id) {
  ...
  openModal(
    ...
    '<label>Product / Category<input id="if_cat" list="if_cats" placeholder="e.g. Electrical, Plumbing" value="' + esc(it ? it.category : '') + '"></label>' +
    '<datalist id="if_cats">' + [...new Set(state.items.map(i => i.category).filter(Boolean))].sort().map(c => '<option value="' + esc(c) + '">').join('') + '</datalist>' +
