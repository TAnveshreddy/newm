...
function renderParties() {
  ...
  let rows = list.map(p => {
    ...
    return '<tr class="rowlink" onclick="openPartyLedger(\'' + p.id + '\')">' +
      ...
      '<td><span class="tag">' + (p.type === 'supplier' ? 'Distributor' : p.type === 'both' ? 'Both' : 'Customer') + '</span></td>' +
      '<td>' + esc(p.gstin || '—') + '</td>' +
  ...
  el('view').innerHTML =
    '<div class="page-head"><h2>Customers &amp; Distributors</h2>' +
...
function openPartyForm(id) {
  ...
  openModal(
    ...
    [['customer', 'Customer'], ['supplier', 'Distributor'], ['both', 'Both']].map(([v, lbl]) => '<option value="' + v + '"' + (p && p.type === v ? ' selected' : '') + '>' + lbl + '</option>').join('') +
    '</select></label>' +
...
function openPartyLedger(id) {
  ...
  el('view').innerHTML =
    '<div class="page-head"><h2><a class="crumb" onclick="go(\'parties\')">Customers &amp; Distributors</a> › ' + esc(p.name) + '</h2>' +
    '<div class="head-actions">' +
