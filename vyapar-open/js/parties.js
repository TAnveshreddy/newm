...
function renderParties() {
  ...
  let rows = list.map(p => {
    ...
    return '<tr class="rowlink" onclick="openPartyLedger(\'' + p.id + '\')">' +
      ...
      '<td><span class="tag">' + (p.type === 'supplier' ? 'Supplier' : p.type === 'both' ? 'Both' : 'Distributor') + '</span></td>' +
      '<td>' + esc(p.gstin || '—') + '</td>' +
  ...
  el('view').innerHTML =
    '<div class="page-head"><h2>Distributors &amp; Suppliers</h2>' +
...
function openPartyForm(id) {
  ...
  openModal(
    ...
    [['customer', 'Distributor'], ['supplier', 'Supplier'], ['both', 'Both']].map(([v, lbl]) => '<option value="' + v + '"' + (p && p.type === v ? ' selected' : '') + '>' + lbl + '</option>').join('') +
    '</select></label>' +
...
function openPartyLedger(id) {
  ...
  el('view').innerHTML =
    '<div class="page-head"><h2><a class="crumb" onclick="go(\'parties\')">Distributors &amp; Suppliers</a> › ' + esc(p.name) + '</h2>' +
    '<div class="head-actions">' +
