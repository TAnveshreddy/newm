...
function syncBootstrap() {
  ...
  if (state.settings.gClientId) loadGsi().catch(() => {});
}

function lastSyncText() {
  const t = num(localStorage.getItem(LAST_SYNC_KEY));
  if (!t) return (state.settings.gClientId || state.settings.syncUrl) ? 'Not synced yet' : '';
  const d = new Date(t);
...
async function syncNow(force) {
  ...
  if (!s.gClientId && !s.syncUrl) { openSyncSetup(); return; }
  setSyncBusy(true);
  try {
    if (s.gClientId) await driveSync(force);
...
function openSyncSetup() {
  ...
  if (s.gClientId) {
    const link = (isFile ? '' : location.origin + location.pathname) + '#gcid=' + s.gClientId;
    body +=
      ...
      '<p class="sub"><strong>On your phone / other devices:</strong> open this link once, then tap 🔄 Sync:</p>' +
      ...
      '<details style="margin-top:10px"><summary style="cursor:pointer;font-weight:600;font-size:13px">Change Google registration (Client ID)</summary>' + ownerSection + '</details>';
  } else {
    body += ownerSection;
  }
