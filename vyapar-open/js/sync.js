...
/* turn a Drive REST error into a message a shop owner can act on */
async function driveHttpError(res, what) {
  let msg = what + ' failed (' + res.status + ')';
  try {
    const body = await res.text();
    if (res.status === 403 && /accessNotConfigured|has not been used|is disabled/i.test(body)) {
      msg = 'Google Drive is not switched ON for this app yet — open console.cloud.google.com → APIs & Services → Library → search "Google Drive API" → Enable, wait 2 minutes, then Sync again';
    } else {
      const j = JSON.parse(body);
      if (j.error && j.error.message) msg = what + ' failed: ' + j.error.message;
    }
  } catch (e) {}
...
async function driveFileId(token) {
  ...
  if (!res.ok) throw await driveHttpError(res, 'Google Drive search');
  const j = await res.json();
...
async function driveDownload(token, fileId) {
  const res = await fetch('https://www.googleapis.com/drive/v3/files/' + fileId + '?alt=media',
    { headers: { Authorization: 'Bearer ' + token } });
  if (!res.ok) throw await driveHttpError(res, 'Google Drive download');
...
async function driveUpload(token, fileId) {
  ...
  if (!res.ok) throw await driveHttpError(res, 'Google Drive upload');
}
